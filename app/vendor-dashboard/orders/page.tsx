...
import { api } from "@/lib/axios";
import { getSocket } from "@/lib/socket";
import { X, ShoppingBag, MapPin, Calendar, Clock, AlertCircle } from "lucide-react";
import VendorSidebar from "@/components/VendorSidebar";
import OrderStatusBadge from "@/components/OrderStatusBadge";
...
export default function VendorOrdersPage() {
  ...
  useEffect(() => {
    ...
    // Listen for rider assignment confirmation
    socket.on("rider_assigned_to_order", (data: any) => {
      console.log("Rider assigned:", data);
      // Update the order in the list
      setOrders((prev) =>
        prev.map((o) =>
          o.id === data.orderId ? { ...o, riderId: data.riderId } : o
        )
      );
    ...
    // Listen for order status changes from other sources
  ...
  }, [isAuthenticated, token, user?.role, selectedStatus, selectedOrder?.id]);
  ...
  useEffect(() => {
    ...
    // Refresh orders every 30 seconds (fallback for Socket.IO)
    const interval = setInterval(() => fetchOrders(selectedStatus), 30000);
    return () => clearInterval(interval);
  }, [isAuthenticated, selectedStatus, router]);

  // Handle order action (accept, reject, start cooking, ready)
  const handleOrderAction = async (
    ...
    try {
      ...
      // ✅ If accepting order, emit socket event to assign rider
      if (action === "accept") {
        const socket = getSocket(token ?? undefined);
        socket.emit("assign_rider_to_order", {
          orderId,
          vendorId: user?.id,
        });
      }
      ...
      console.error(`Error performing action ${action}:`, err);
      setError(
        err.response?.data?.message || `Failed to ${action} order`
      );
    } finally {
        ...
        <header className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-xs text-yellow-500 font-bold font-mono">
                VENDOR
              </p>
            </div>
            <div className="w-10 h-10 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-bold shadow-sm">
                  ...
                  {orders.map((order) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-600">
                        {order.riderId ? `#${order.riderId}` : "Pending"}
                      </td>
                      ...
                      <td className="px-6 py-4 flex gap-2">
                        {order.status === "PLACED" && (
                          <>
                            <button
                              onClick={(e) =>
                                handleOrderAction(e, order.id, "accept")
                              }
                              disabled={actionLoading}
                              className="px-3 py-1.5 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white rounded font-bold text-xs shadow-sm transition disabled:cursor-not-allowed"
                            >
                              Accept
                            </button>
                            <button
                              onClick={(e) =>
                                handleOrderAction(e, order.id, "reject")
                              }
                              disabled={actionLoading}
                              className="px-3 py-1.5 bg-red-600 hover:bg-red-700 disabled:bg-gray-300 text-white rounded font-bold text-xs shadow-sm transition disabled:cursor-not-allowed"
                            >
                              Reject
                            </button>
                          </>
                        ...
                        {order.status === "ACCEPTED" && (
                          <button
                            onClick={(e) =>
                              handleOrderAction(e, order.id, "preparing")
                            }
                            disabled={actionLoading}
                            className="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 text-white rounded font-bold text-xs shadow-sm transition disabled:cursor-not-allowed"
                          >
                            Start Cooking
                          </button>
                        )}
                        {order.status === "PREPARING" && (
                          <button
                            onClick={(e) =>
                              handleOrderAction(e, order.id, "ready")
                            }
                            disabled={actionLoading}
                            className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-300 text-white rounded font-bold text-xs shadow-sm transition disabled:cursor-not-allowed"
                          >
                            Mark Ready
                          </button>
                        )}
              ...
              <div className="flex-grow overflow-y-auto space-y-6 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-gray-900 mt-1 block uppercase">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-gray-900 mt-1 block flex items-center gap-1">
                      ...
                      {selectedOrder.riderId ? `#${selectedOrder.riderId}` : "Pending"}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 border-b border-gray-100 pb-4 text-xs text-gray-500 font-medium">
                  ...
                  <div className="flex justify-between">
                    <span>Delivery Fee</span>
                    <span className="font-bold text-gray-900">
                      ₹{parseFloat(selectedOrder.deliveryFee).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>GST (5%)</span>
                ...
                <div className="pt-4 flex gap-3">
                  {selectedOrder.status === "PLACED" && (
                    <>
                      <button
                        onClick={(e) =>
                          handleOrderAction(e, selectedOrder.id, "accept")
                        }
                        disabled={actionLoading}
                        className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition disabled:cursor-not-allowed"
                      >
                        {actionLoading ? "..." : "Accept Order"}
                      </button>
                      <button
                        onClick={(e) =>
                          handleOrderAction(e, selectedOrder.id, "reject")
                        }
                        disabled={actionLoading}
                        className="flex-1 bg-red-600 hover:bg-red-700 disabled:bg-gray-300 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition disabled:cursor-not-allowed"
                      >
                  ...
                  {selectedOrder.status === "ACCEPTED" && (
                    <button
                      onClick={(e) =>
                        handleOrderAction(e, selectedOrder.id, "preparing")
                      }
                      disabled={actionLoading}
                      className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition disabled:cursor-not-allowed"
                    >
                  ...
                  {selectedOrder.status === "PREPARING" && (
                    <button
                      onClick={(e) =>
                        handleOrderAction(e, selectedOrder.id, "ready")
                      }
                      disabled={actionLoading}
                      className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-300 text-white font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-sm transition disabled:cursor-not-allowed"
                    >
